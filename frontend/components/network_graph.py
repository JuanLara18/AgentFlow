...
def display_organization_graph(hierarchy_depth=3, span_of_control=5, centralization=0.5, 
    ...
    try:
        ...
        try:
            ...
            st.error(f"Error calculating graph layout: {str(e)}")
            # Fallback to a simpler layout if hierarchy_pos fails
        ...
        st.error(f"Error generating organization graph: {str(e)}")
        st.info("Unable to display organization graph. Please try adjusting the parameters.")
...
def _hierarchy_pos(G, root, width=1., vert_gap=0.2, vert_loc=0, xcenter=0.5,
    ...
    if parent is not None:
        # Only try to remove parent if it exists in the children list
